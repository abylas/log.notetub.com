...
/**
 * This is the model class for table "{{notespic}}".
 *
 * The followings are the available columns in table '{{notespic}}':
 * @property string $id
 ...
 */
class Notespic extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, name, extension, path, filename, byteSize, mimeType, created, notes_id', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function beforeSave()
    {
		...
		$filePath = "files/images/profilePic/".sha1(Yii::app()->user->id); // so this should be like bitpic/  
           ...
           //	$this->image=file_get_contents($file->tempName);
           	$this->created=time();           	
            $this->notes_id=Yii::app()->user->id; //Yii::app()->getModule('user')->id;
           	...
           	if(!file_exists($filePath))
				if(!$this->createDirectory($filePath))
					fb('Notes Pic error,Failed to save image! Directory could not be created.');
